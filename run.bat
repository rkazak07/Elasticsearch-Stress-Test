...
python es-perf-test.py --es_ip %host%  --indices %indices% --documents %documents% --client_conn %clients% --duration %seconds% --shards %shards% --replicas %replicas%  --bulk_number %bulk_size% --not-green --max-fields-per-document %max_fields_per_doc% --max-size-per-field %max_size_per_field% --stats-frequency %stats_frequency% --user %username% --pass %password% --no-verify
pause
